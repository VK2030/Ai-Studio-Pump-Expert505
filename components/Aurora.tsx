...
const Aurora: React.FC<AuroraProps> = (props) => {
  const { 
    ...
    color2, 
    color3, 
    colorStops: propColorStops, 
    amplitude = 1.0, 
    blend = 0.5,
    speed = 1.0 
  } = props;

  // Adapt color1, color2, color3 to colorStops if present
  const colorStops = propColorStops || [
    color1 || '#486099',
    color2 || '#504fb4',
    color3 || '#583ec1'
  ];
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      const stops = propsRef.current.colorStops || [
        propsRef.current.color1 || '#486099',
        propsRef.current.color2 || '#504fb4',
        propsRef.current.color3 || '#583ec1'
      ];
